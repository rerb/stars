"""
    Provides QuerySetSequence, a class that implements most of the
    QuerySet interface, but can contain multiple models.

    http://djangosnippets.org/snippets/1933/
"""
...
class IableSequence(object):
    ...
    def __getitem__(self, key):
        ...
        assert ((not isinstance(key, slice) and (key >= 0))
                or (isinstance(key, slice) and
                    (key.start is None or key.start >= 0)
