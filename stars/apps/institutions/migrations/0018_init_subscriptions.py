...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for i in orm.Institution.objects.order_by('name'):
            ...
            for ss in orm['submissions.SubmissionSet'].objects.filter(institution=i).order_by('-date_registered').exclude(is_locked=True).exclude(is_visible=False):
                print "%s - %s" % (ss.date_registered, ss.submission_deadline)
                
                subscription_end = ss.date_registered + td
                ...
                if ss.status == 'r':
                    print "Rated"
                    print "subscription: %s - %s" % (ss.date_registered, subscription_end)
                    if subscription_end > datetime.date.today():
                        print "****NEEDS EXTENSION****"
                        need_extension.append(ss)
                else:
                    if ss.submission_deadline > datetime.date.today():
                        print "new subscription: %s - %s" % (ss.date_registered, subscription_end)
                break
