...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """
            The migration updates the slugs since I didn't include
            the state in the slugify 
        """
        for institution in orm.Institution.objects.all():
            try:
                slug_base = "%s-%s" % (institution.profile.org_name,
                                       institution.profile.state.lower())
                institution.slug = slugify(slug_base)
                institution.save()
            except Exception, e:
                pass
