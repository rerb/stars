...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for i in orm.Institution.objects.all():
            if orm['submissions.SubmissionSet'].objects.filter(status='r').filter(institution=i).count() > 0:
                i.rated_submission = orm['submissions.SubmissionSet'].objects.filter(status='r').filter(institution=i).order_by('date_submitted')[0]
                i.save()
