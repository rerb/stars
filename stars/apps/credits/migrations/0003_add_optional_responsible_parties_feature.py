...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        feature = orm.IncrementalFeature.objects.create(
            key='optional_responsible_parties',
            description='Optional responsible parties')
        for credit_set in orm.CreditSet.objects.filter(
                version__startswith='2.1'):
            credit_set.supported_features.add(feature)

    def backwards(self, orm):
        "Write your backwards methods here."
        try:
            feature = orm.IncrementalFeature.objects.get(
                key='optional_responsible_parties')
            feature.delete()
        except orm.IncrementalFeature.DoesNotExist:
            pass
