...
from stars.apps.tool.credit_editor.decorators import *
...
def credit_set_detail(request, creditset_id):
    ...
    template = "tool/credit_editor/set_detail.html"
...
def add_credit_set(request):
    ...
    template = 'tool/credit_editor/new_creditset.html'
    context = {
...
def credit_set_confirm_unlock(request, creditset_id):
    ...
    template = "tool/credit_editor/confirm_unlock.html"
    context.update({'confirm_form': form})
    return respond(request, template, context)
...
def credit_set_locked(request, creditset_id):
    ...
    """
    context = _get_creditset_context(creditset_id)
    return respond(request, "tool/credit_editor/set_locked.html", context)
...
def credit_set_ratings(request, creditset_id):
    ...
    template = "tool/credit_editor/set_ratings.html"
...
def delete_rating(request, creditset_id, rating_id):
    ...
    template = 'tool/credit_editor/credits/applicability_delete.html'
    context.update({'object_class':'Rating', 'object':rating,'confirm_form': form,})
    return respond(request, template, context)
...
def category_detail(request, creditset_id, category_id):
    ...
    template = "tool/credit_editor/category.html"
    context.update({'object_form': object_form, 
                    'object_ordering': object_ordering, 
...
def delete_category(request, creditset_id, category_id):
    ...
    template = "tool/credit_editor/delete_object.html"
    context.update({'object_class':'Category', 'object':category, 'confirm_form': form, 'dependent_depth': 3})
    return respond(request, template, context)
...
def add_category(request, creditset_id):
    ...
    template = 'tool/credit_editor/new_category.html'
    context.update({"object_form": object_form})
    return respond(request, template, context)
...
def subcategory_detail(request, creditset_id, category_id, subcategory_id):
   ...
   # Build the form for adding a new subcategory
    ...
    template = "tool/credit_editor/subcategory.html"
...
def delete_subcategory(request, creditset_id, category_id, subcategory_id):
    ...
    template = "tool/credit_editor/delete_object.html"
    context.update({'object_class':'Subcategory', 'object':subcategory, 'confirm_form': form, 'dependent_depth': 2})
    return respond(request, template, context)
...
def add_subcategory(request, creditset_id, category_id):
    ...
    template = 'tool/credit_editor/new_subcategory.html'
    context.update({"object_form": object_form})
    return respond(request, template, context)
...
def delete_credit(request, creditset_id, category_id, subcategory_id, credit_id):
    ...
    template = "tool/credit_editor/delete_object.html"
    context.update({'object_class':'Credit', 'object':context['credit'], 'confirm_form': form})
    return respond(request, template, context)
...
def add_credit(request, creditset_id, category_id, subcategory_id):
    ...
    template = 'tool/credit_editor/new_credit.html'
    context.update({"object_form": object_form})
    return respond(request, template, context)
...
def add_t2_credit(request, creditset_id, category_id, subcategory_id):
    ...
    template = 'tool/credit_editor/new_t2_credit.html'
    context.update({"object_form": object_form})
    return respond(request, template, context)
...
def add_field(request, creditset_id, category_id, subcategory_id, credit_id):
    ...
    template = 'tool/credit_editor/credits/new_field.html'
    context.update({"object_form": object_form})
    return respond(request, template, context)
...
def field_detail(request, creditset_id, category_id, subcategory_id, credit_id, field_id):
    ...
    # Build and process the form for the field (slightly complicated - see docs in helper method)
    (field_form, redirect) = _process_field_form(request, field)
    if redirect:
        return redirect
...
def _process_field_form(request, field):
    """ Helper method to safely save changes to a documentation field and return the field form """
    # We need to confirm changes to the field type if there are active submissions for this field - see ticket #217
    redirect = None
    from copy import copy
    old_field = copy(field)  
    # Build and process the form as if everything is OK
    # As an alternate, we could check the form before saving, but its more code, less generic, so we'll forgo efficiency...
    (field_form, saved) = form_helpers.basic_save_form(request, field, 'field_%d' % field.id, DocumentationFieldForm)
    
    if saved and old_field.type != field.type and old_field.num_submissions() > 0:
        # put the type back the way it was and seek confirmation...
        new_field_type = field.type
        field.type = old_field.type
        field.save()
        redirect = HttpResponseRedirect( '%schange_field_type/?new_field_type=%s'%(field.get_edit_url(), new_field_type) )
    
    return (field_form, redirect)
    
@user_is_staff
@creditset_is_unlocked
@transaction.commit_on_success
def change_field_type(request, creditset_id, category_id, subcategory_id, credit_id, field_id):
    """
        Get confirmation that the type of a Documentation Field should be changed.
        This is really a helper view for _process_Field_form(), takes one GET parameter: new_field_type
    """
    context = _get_doc_field_context(creditset_id, category_id, subcategory_id, credit_id, field_id)
    field = context['field']
    new_field_type = request.GET.get('new_field_type', field.type)
    
    (form, confirmed) = form_helpers.confirm_form(request, field)       
    if confirmed is not None:
        if confirmed:
            num_submissions = field.num_submissions()  # grab it now before saving new field type!
            # Delete the old submission data, so we don't leave any orphaned data around.
            for field_submission in field.get_active_submissions():
                field_submission.delete()
                
            # Required fields with no data cannot be part of a complete submission - make it so
            if field.is_required():
                for credit_submission in field.credit.get_complete_submissions():
                    credit_submission.mark_as_in_progress()
                    credit_submission.save()

            field.type = new_field_type
            field.save()
           
            flashMessage.send('Field type updated - %s submissions were potentially affected.'%num_submissions, flashMessage.SUCCESS)
        return HttpResponseRedirect(field.get_edit_url())
    
    template = 'tool/credit_editor/credits/change_field_type.html'
    context.update({'field':field, 'new_field_type': new_field_type, 'confirm_form': form,})
    return respond(request, template, context)
...
def delete_field(request, creditset_id, category_id, subcategory_id, credit_id, field_id):
    ...
    template = 'tool/credit_editor/credits/delete_field.html'
    context.update({'object_class':'Reporting Field', 'object':context['field'],'confirm_form': form,})
    return respond(request, template, context)
...
def delete_choice(request, creditset_id, category_id, subcategory_id, credit_id, field_id, choice_id):
    ...
    #@todo: rename the delete form to make it more generic - and override Choice.delete() to manage the submission objects.
    template = 'tool/credit_editor/credits/applicability_delete.html'
    context.update({'object_class':'Choice', 'object':choice,'confirm_form': form,})
    return respond(request, template, context)
...
def delete_applicability(request, creditset_id, category_id, subcategory_id, credit_id, reason_id):
    ...
    template = 'tool/credit_editor/credits/applicability_delete.html'
    context.update({'object_class':'Applicability Reason', 'object':reason,'confirm_form': form,})
    return respond(request, template, context)
...
def formula_test_case(request, creditset_id, category_id, subcategory_id, credit_id, test_case_id):
    ...
    template = 'tool/credit_editor/credits/formula_test_case.html'
...
def add_formula_test_case(request, creditset_id, category_id, subcategory_id, credit_id):
    ...
    template = 'tool/credit_editor/credits/new_test_case.html'
...
def add_units(request):
    """
        Provides a pop-up form for adding a new Units descriptor
        and updates the dropdown on the credit field editor
    """
     # create the form for editing a Unit
    new_unit = Unit()
    (units_form, saved) = form_helpers.basic_save_new_form(request, new_unit, 'new_unit', UnitForm)
    if saved:
        context = {'id': new_unit.id, 'name': new_unit,}
        return respond(request, "tool/credit_editor/units_redirect.html", context)
    
    context = {'units_form': units_form,}

    return respond(request, "tool/credit_editor/units.html", context)
